    ...
    try {
      ...
      container('nodejs-builder') {
        ...
        stage("Install") {
          ...
          sh "mkdir -p /var/cache/${appName}/_cache"
          sh "cp -r /var/cache/${appName}/* ."
          sh "npm install"
          sh "rm -rf /var/cache/${appName}/* && cp -r node_modules /var/cache/${appName}/"
        }
      ...
      container('kubectl') {
        stage('Deploy To K8S') {
          ...
          sh "helm upgrade $deployment ./k8s/style-guide -f $varsFile --set image.repository=$image --set image.tag=$tag"
